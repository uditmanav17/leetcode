...
class Solution:
    def minCameraCover(self, root: Optional[TreeNode]) -> int:
        ...
        def dfs(node: Optional[TreeNode]) -> int:
            ...
            curr = min(node.left.val if node.left else float('inf'), 
            ...
            if curr == 0:
                ...
                node.val = 2
            # if curr == float('inf'), the current node is a leaf node; 
            # let the parent node monitor this node
            # if curr == 2, all child nodes are being monitored; 
            # treat the current node as a leaf node
            return res
